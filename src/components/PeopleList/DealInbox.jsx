...
class DealInbox extends Component {
  constructor(props) {
    super(props);
    this.state = {
      channels: [],
  ...
  infiniteScroll = async () => {
    if (this.props.deviceType==='MOBILE' ) {
      if (document.getElementById('dealInbox').scrollTop + (window.innerHeight+10) >= (document.getElementById('dealInbox').scrollHeight)) {
      ...
      this.offsetHeight = document.getElementById('contentDealList').offsetHeight;
      if (this.offsetHeight/2 < (document.getElementById('contentDealList').scrollTop) && this.state.page < this.maxtotalPage -1 ) {
        this.offsetHeight = document.getElementById('contentDealList').scrollTop;
        await this.setState({
          page: this.state.page + 1,
        });
        await this.loadChannelList(this.state.page);
  ...
  async componentDidUpdate(prevProps) {
    if (this.props.socket !== null && prevProps.socket !== this.props.socket) {
      ...
      // if (document.getElementById("contentDealList")) {
      //   document
      //   .getElementById("contentDealList")
      //   .addEventListener("scroll", this.infiniteScroll);
      // }
    } else {
      if (this.props.deviceType==='MOBILE' ) {
        document.getElementById('dealInbox')
            .addEventListener('scroll', this.infiniteScroll);
      } else {
        if (document.getElementById('contentDealList')) {
          document
    ...
    if (this.props && this.props.group && prevProps.group !== this.props.group) {
      await this.updateGroupList(this.props.group);
    }
  ...
  openDealAccordian() {
    this.props.accordianOpen('quote');
  }
  closeDealAccordian() {
  ...
  // onClick={()=>this.props.accordianOpen('quote')}
  ...
  render() {
    return (
      <div className="deal-inbox customScroll" id="dealInbox">
        <Collapsible trigger="Deal Inbox" open={
          (URL_DATA.userType == 'app') ?
          this.props.chatOpen == 'quote' ? true : false :
          true}
        onTriggerOpening={()=>this.openDealAccordian()} onTriggerClosing={()=>this.closeDealAccordian()}>
          <div className="lists" id="contentDealList">
                  ...
                  >
                    <div onClick={() =>
                      ...
                      <a href="#" className="user-text " title={channel.groupName}>
                        {
                        channel && channel.groupName && this.props && this.props.groupChannel && this.props.groupChannel.groupName && this.props.groupChannel.groupName == channel.groupName ?
                          <span className="active">{channel.groupName}</span> : <span>{channel.groupName}</span>
                        }

                        {this.props.groupChannel.groupName ?
                        ...
                        <span className="descText">{channel.message}</span>
                      </a>
                    </div>
                    {/* {
                      this.props && this.props.unreadCount && this.props.unreadCount.length > 0 ?
                        this.props.unreadCount.findIndex(j => j.groupName == channel.groupName) > -1 && this.props.channel.groupName != channel.groupName ?
                          <span className="chat-counter">{this.props.unreadCount[this.props.unreadCount.findIndex(j => j.groupName == channel.groupName)].count} </span> : ""
                        :
                        channel.unread > 0 ?
                          this.props && this.props.socket ?
                            "" : <span className="chat-counter">{channel.unread}</span> : ""
                    } */}
            ...
            {
               this.props.group && this.state.channels && this.state.channels.length == 0 && !this.props.notSearchgroup ? 'No Group Found' :
                ''}
